...
BeforeAll {
    ...
    function ThenUseApprovedVerbs
    {
        ...
        $verbs =
            Get-Command -Module 'XWikiAutomation'|
...
Describe 'XWikiAutomation' {
    It 'should have about help topic' {
        GivenModuleImported
        ThenHelpTopic 'about_XWikiAutomation' -Exists
    }
    ...
    It 'should have a help topic for each command' {
        GivenModuleImported
        foreach( $cmd in (Get-Command -Module 'XWikiAutomation' -CommandType Function,Cmdlet,Filter))
        {
